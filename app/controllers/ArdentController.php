...
class ArdentController extends BaseController {
	...
	{		
		...
		// Load the filters with null
		$id_filterValues = array();
		foreach($this->model->filters as $key => $value)
			$id_filterValues[$key] = null;

		// Load the wheres with null
		$id_whereValues = array();
		foreach($this->model->wheres as $key => $value)
			$id_whereValues[$key] = null;

		// Load the wheres temp values
		$id_filterTempValues = array();
		foreach($this->model->filtersTemp as $key => $value)
			$id_filterTempValues[$key] = null;
		...
		foreach($this->model->filters as $key => $value)
		{
				...
				if ($key == $keyFiltersWhere)
				{
					if ($valueFiltersWhere)
					{
						foreach($valueFiltersWhere as $k => $v)
						{
							...
							if (array_key_exists($v, $id_filterTempValues))
							{
								$data = $data->where($v, '=', $id_filterTempValues[$v]);
								$row = $row->where($v, '=', $id_filterTempValues[$v]);
							}
						}	
					}
				}
			...
			$arrayTemp = array();
			if (array_key_exists('id_organization', $id_filterValues) && $value == 'organizations')
			{
				if (Auth::user()->id != 1)
				{
					$data = $data->where('id', '=', Auth::user()->id_organization);				
					$row = $row->where('id', '=', Auth::user()->id_organization);
				}
			}
			if (array_key_exists('id_database', $id_filterValues) && $value == 'databases')
			{
				if (Auth::user()->id != 1)
				{
					$data = $data->whereIn('id', Auth::user()->getDatabases());
					$row = $row->whereIn('id', Auth::user()->getDatabases());
				}
			}
			...
			{
				$row = $row->first();
				if ((Auth::user()->id == 1 || $key != 'id_organization') && count($data) > 1)
				{
					$arrayTemp = array('0' => 'All ' . $value);
					$valueFiltersWhere = 0;
					if (array_key_exists($key, $id_filterTempValues)) 
						$id_filterTempValues[$key] = 0;
				}
				else
				{
					$valueFiltersWhere = $row->id;
					if (array_key_exists($key, $id_filterTempValues)) 
					{
						$id_filterTempValues[$key] = $row->id;
					}
				}
			}
			else
			{
				...
				$valueFiltersWhere = -1;
				$id_filterTempValues[$key] = -1;
			}
			Session::put($key.'Selected', $valueFiltersWhere);
			$data = $arrayTemp + $data;
			$dataArray[$key] = $data;
		}
	...
	public function getSearch()
	{
		...
		// 		$data = $data->where('id', '=', Auth::user()->id_organization);
		// 	}
		// 	if (array_key_exists('id_database', $id_filterValues))
		// 	{
		// 		if (Auth::user()->id != 1)
		// 			$data = $data->whereIn('id', Auth::user()->getDatabases());
		// 	}
		...
		foreach($id_whereValues as $key => $value)
		{
			if (isset($id_filterValues[$key]) && $id_filterValues[$key])
				$modelData = $modelData->where($key, '=', $id_filterValues[$key]);
			elseif (is_array($this->model->wheres[$key]))
				$modelData = $modelData->whereIn($key, $this->model->wheres[$key]);
			elseif ($value <> 0)
				$modelData = $modelData->where($key, '=', $value);
		}
	...
	public function editDetail()
	{
        ...
        return View::make($modelName. '.edit', compact('model', 'modelName', 'tableName', 'organizations'));
	}
	...
	public function back()
	{
		...
		{
			$modelName = Session::get('modelName');	
			$id_fieldName = 'id_' . str_singular($modelName);
			$tableName = str_plural(lcfirst($modelName));
			$instanceData = Session::get('instanceData');
			if ($this->model->id)
			{
				$id_metaData = $instanceData['id_metaData'];
				$instanceData[$id_fieldName] = $this->model->id;
				$instanceData[$tableName] = $this->model->getNameAttribute();
				Helper::deleteDependent($id_fieldName, $id_metaData, $instanceData);
				Session::put('instanceData', $instanceData);
			}
		...
		return Redirect::route(Session::get('nestedRoutePath'));
